...
import { Wrench, Building2, Sparkles } from "lucide-react";
import { cmsStore } from "../lib/cmsStore";
import { ToolItem, ClientBrand } from "../lib/cmsTypes";

export default function Brands() {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, margin: "-80px" });
  const [tools, setTools] = useState<ToolItem[]>(() => 
    cmsStore.getTools().filter(t => t.visible !== false)
  );
  ...
  useEffect(() => {
    const handleUpdate = () => {
      setTools(cmsStore.getTools().filter(t => t.visible !== false));
  ...
  if (tools.length === 0 && clients.length === 0) return null;
  ...
  return (
    <section ref={ref} id="brands" className="py-16 sm:py-20 relative overflow-hidden bg-primary">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 space-y-16 sm:space-y-20 relative z-10">
        {/* Client Brands Showcase (if available) */}
        {clients.length > 0 && (
          <div>
            <motion.p
        ...
        {tools.length > 0 && (
          <div>
            <motion.p
              ...
              className="text-center text-[9px] sm:text-[10px] uppercase tracking-[0.3em] text-text-muted font-mono mb-10"
            >
              Industry Standard Software & Toolkits
            </motion.p>

            {/* Brand grid */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-accent/10 border border-accent/10 rounded-2xl sm:rounded-3xl overflow-hidden shadow-2xl">
              {tools.map((brand, i) => (
                <motion.div
                  ...
                  transition={{ duration: 0.4, delay: i * 0.05 }}
                  className="group relative bg-secondary/60 px-4 sm:px-6 py-6 sm:py-8 flex flex-col items-center justify-center gap-1.5 sm:gap-2 hover:bg-secondary transition-colors duration-300 cursor-default text-center"
                >
                  {/* Hover accent glow */}
                  ...
                  {/* Brand name */}
                  <span className="font-display text-base sm:text-xl font-bold text-text-pure group-hover:text-accent transition-colors duration-300 tracking-tight truncate max-w-full">
                    {brand.name}
                  </span>

                  {/* Category tag */}
                  <span className="text-[8px] sm:text-[10px] uppercase tracking-widest text-text-muted font-mono">
                    {brand.category}
                  </span>
